...
export async function handleOtterTool(name, args) {
    ensureDirectories();
    switch (name) {
        case 'otter_get_transcript': {
            ...
            if (search) {
                ...
                transcripts = transcripts.filter(t => t.title.toLowerCase().includes(searchLower) ||
                    t.transcript.toLowerCase().includes(searchLower) ||
                    (t.summary && t.summary.toLowerCase().includes(searchLower)));
            }
            // Filter by date
            if (date) {
                transcripts = transcripts.filter(t => t.date && t.date.startsWith(date));
            }
            // Return last N
        ...
        case 'otter_search_insights': {
            ...
            if (dateRange) {
                transcripts = transcripts.filter(t => {
                    ...
                    if (dateRange.start && tDate < new Date(dateRange.start))
                        return false;
                    if (dateRange.end && tDate > new Date(dateRange.end))
                        return false;
                    return true;
                });
            }
...
//# sourceMappingURL=otter.handler.js.map