...
export async function handleWorkflowTool(name: string, args: any) {
  switch (name) {
    ...
    case 'workflow_schedule_meeting': {
      ...
      const slots = (slotsResult && Array.isArray((slotsResult as any).slots)) ? (slotsResult as any).slots : [];
      if (slots.length === 0) {
        return {
          status: 'error',
    ...
    case 'workflow_daily_briefing': {
      ...
      const unreadFrom = args?.unreadFrom as string | undefined;
      let emails: any = null;
      if (unreadFrom) {
    ...
    case 'workflow_followup_nudge': {
      ...
      const drafts = threads.map((t: any) => ({
        threadId: t.id,
        draft: {
          subject: subjectT,
          bodyPreview: bodyT
...
export async function handleWorkflowScheduleAndInvite(args: any) {
